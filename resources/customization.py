#! /usr/bin/env python
#  -*- coding: utf-8 -*-
#
# This file is part of altaro_exporter

__intname__ = "altaro_exporter.customization"
__author__ = "Orsiris de Jong"
__copyright__ = "Copyright (C) 2022-2026 NetInvent"
__license__ = "GPL-3.0-only"
__build__ = "2026012301"
__version__ = "1.3.1"


import os

# Windows Program name for scheduled task
PROGRAM_NAME = "NetPerfect VMBackup Exporter"

# Windows executable file info
COMPANY_NAME = "NetInvent"
COPYRIGHT = "NetInvent 2024-2026"
FILE_DESCRIPTION = "HornetSecurity VM Backup exporter for Prometheus"
TRADEMARKS = "NetInvent (C)"
PRODUCT_NAME = "VMBackup Exporter for Prometheus"
# This is what the name will be when querying / endpoint
SHORT_PRODUCT_NAME = "altaro_exporter"

OEM_ICON = b""
